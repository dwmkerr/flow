...
- [x] ⌘ + ?: Show this help
- [ ] Edit ~/.flow.md to customize your shortcuts

## Example Section

- [x] ⌘ + Space: Example shortcut
- [ ] ⌥ + Enter: Hidden shortcut example

## Terminal

- [x] ⌃ + L: Clear screen
- [x] ⌃ + R: Search command history
"""

def init_flow_config():
    """Initialize a new flow configuration file."""
    config_path = os.path.expanduser("~/.flow.md")
    
    if os.path.exists(config_path):
        response = input(f"{config_path} already exists. Overwrite? [y/N] ").lower()
        if response != 'y':
            print("Aborted.")
            return
    ...
    try:
        ...
        print("Customize your shortcuts by editing this file.")
    except IOError as e:
...
def print_help():
    ...
    print("  --init       Create a new flow configuration file at ~/.flow.md")
    sys.exit(0)
